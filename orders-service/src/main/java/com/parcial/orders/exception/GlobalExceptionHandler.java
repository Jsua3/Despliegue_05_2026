package com.parcial.orders.exception;

import com.parcial.orders.dto.ErrorResponse;
...
public class GlobalExceptionHandler {
    ...
    private ResponseEntity<ErrorResponse> error(HttpStatus status, String message, HttpServletRequest request) {
        return ResponseEntity.status(status)
                .body(new ErrorResponse(LocalDateTime.now(), status.value(), status.getReasonPhrase(), message, request.getRequestURI()));
    }
}
